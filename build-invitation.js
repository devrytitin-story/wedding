  ...
  <div class="sec-glass-card" style="text-align: center;">
    ...
    <div style="text-align: center; margin-top: 14px;" data-rv="fade">
      <button id="btn-toggle-gift" class="gift-toggle-btn" onclick="toggleGiftBox()" data-cursor>
        ...
        <span id="btn-toggle-gift-text">Kirim Tanda Kasih</span>
    ...
    <div id="gift-box-wrapper" class="gift-box-wrapper">
      <div class="gift-grid">
        ...
        <div class="gift-card">
          <div>
            <span class="gift-bank-badge">REKENING BANK</span>
...
window.toggleGiftBox = function() {
  ...
  if (!isOpen) {
    ...
    if (btnText) btnText.textContent = 'Kirim Tanda Kasih';
...
let toastTimer = null;
function showGiftToast(msg, type = 'gold') {
  ...
  if (type === 'success') {
    ...
    toast.classList.add('gift-toast-success');
    toast.classList.remove('gift-toast-warn');
  } else if (type === 'warn') {
    ...
    toast.classList.add('gift-toast-warn');
    toast.classList.remove('gift-toast-success');
  } else {
    toast.classList.remove('gift-toast-warn', 'gift-toast-success');
  }
...
// F. Dynamic Wishes & RSVP System with LocalStorage & Google Sheets Sync
const GSHEET_WEBAPP_URL = 'https://script.google.com/macros/s/AKfycbxtGVFOY2ug_OyWfzfnA_vNjVrjoM6cb0l1gJxkIz8fatn7LSKQNc04fHUu277caHPfbw/exec';
window.GSHEET_WEBAPP_URL = GSHEET_WEBAPP_URL;
...
function renderWishesDOM(wishesList) {
  const listEl = document.getElementById('wishes-list');
  const cntEl = document.getElementById('wishes-count');
  if (cntEl) cntEl.textContent = wishesList.length;

  if (listEl) {
    if (!wishesList.length) {
      listEl.innerHTML = '<div style="color: rgba(255,255,255,0.5); font-size: 12px; text-align: center; padding: 20px;">Belum ada doa. Jadilah yang pertama memberikan doa restu!</div>';
      return;
    }
...
function loadWishes() {
  ...
  // Tampilkan data lokal/default secara instan terlebih dahulu
  let allWishes = [...stored, ...DEFAULT_WISHES];
  renderWishesDOM(allWishes);

  // Ambil data realtime dari Google Spreadsheet (doGet)
  const targetSheetUrl = window.GSHEET_WEBAPP_URL || 'https://script.google.com/macros/s/AKfycbxtGVFOY2ug_OyWfzfnA_vNjVrjoM6cb0l1gJxkIz8fatn7LSKQNc04fHUu277caHPfbw/exec';
  if (targetSheetUrl) {
    fetch(targetSheetUrl)
      ...
      .then(res => {
        if (res && res.status === 'success' && Array.isArray(res.data) && res.data.length > 0) {
          ...
          // Prioritaskan data lokal terbaru
          stored.forEach(item => {
            const key = (item.name + '|' + item.msg).trim().toLowerCase();
            if (!seen.has(key)) {
              seen.add(key);
              merged.push(item);
            }
          ...
          // Masukkan data dari Google Spreadsheet
          ...
          // Masukkan default wishes sebagai pelengkap
          DEFAULT_WISHES.forEach(item => {
            const key = (item.name + '|' + item.msg).trim().toLowerCase();
            if (!seen.has(key)) {
              seen.add(key);
              merged.push(item);
          ...
          renderWishesDOM(merged);
        }
      })
      .catch(err => console.warn('Sync GSheet wishes error:', err));
  }
}
...
function saveWishData(name, status, count, msg) {
  ...
  // Kirim background request otomatis ke Google Sheets (Paten)
  const targetSheetUrl = window.GSHEET_WEBAPP_URL || 'https://script.google.com/macros/s/AKfycbxtGVFOY2ug_OyWfzfnA_vNjVrjoM6cb0l1gJxkIz8fatn7LSKQNc04fHUu277caHPfbw/exec';
  try {
    fetch(targetSheetUrl, {
      method: 'POST',
    ...
    }).catch(e => console.warn('GSheet sync error:', e));
  } catch(e) {}
...
window.handleRsvpSubmit = function(e) {
  ...
  showGiftToast('Doa tersimpan! Mengarahkan ke WhatsApp...', 'success');
  setTimeout(() => {
    window.open(waUrl, '_blank');
  }, 400);
